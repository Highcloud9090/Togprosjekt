...
void sendBuffer() {
  ...
  for(int i=0;i<ANTALL;i++){
    ...
    aksel["x"]=buffer[i].ax; aksel["y"]=buffer[i].ay; aksel["z"]=buffer[i].az;

    JsonObject gyro = m.createNestedObject("gyro");
    gyro["x"]=buffer[i].gx; gyro["y"]=buffer[i].gy; gyro["z"]=buffer[i].gz;
  }
  ...
  if(state==RADIOLIB_ERR_NONE) Serial.println("Buffer sendt!");
  else { Serial.print("Feil ved sending: "); Serial.println(state); }
}
...
void setup() {
  ...
  Serial.println("MPU6050 startet");

  delay(100);

  // Init RTC
  Serial.println("Starter RTC");
  if(!rtc.begin()){
  ...
  if(!rtc.initialized() || rtc.lostPower()){
    Serial.println("RTC ikke initialisert, setter tid til kompileringstid");
    rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
  }
  Serial.println("RTC startet");

  // LoRa
  Serial.println("Starter Radio");
  ...
  Serial.println("Vellykket radio start");
