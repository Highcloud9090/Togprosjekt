...
const char* password = "";
const char* mqtt_server = "10.22.100.166"; //change!
...
void setup() {
  Serial.begin(115200);
  while (!Serial);
  msg = "Serial started (" + String(millis()) + ')';
  Serial.println(msg);
...
void loop() {
  ...
  if (millis() - last > 1000) {
    ...
    client.publish("devices/esp32-1/accel/x", String(axs2).c_str());
    client.publish("devices/esp32-1/accel/y", String(ays2).c_str());
    client.publish("devices/esp32-1/accel/z", String(azs2).c_str());
    client.publish("devices/esp32-1/gyro/x",  String(gxdps).c_str());
    client.publish("devices/esp32-1/gyro/y",  String(gydps).c_str());
    client.publish("devices/esp32-1/gyro/z",  String(gzdps).c_str());
  }
}
